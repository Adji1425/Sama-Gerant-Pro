...
def home(request):
    ...
    # Populaires — les 8 produits les PLUS VENDUS (quantité totale sur des
    # commandes livrées), triés strictement par quantité vendue décroissante.
    # Un produit peut apparaître à la fois dans "Nouveautés" et "Populaires"
    # si c'est un ajout récent qui se vend déjà très bien — c'est voulu et
    # honnête (on ne veut pas cacher un vrai best-seller). S'il y a moins de
    # 8 produits avec au moins 1 vente, la section affiche juste moins de 8
    # produits plutôt que de compléter avec des produits jamais vendus.
